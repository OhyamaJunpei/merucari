...
//				.antMatchers("/","/login", "/showItem/index").permitAll() // ログインが必要なのは、注文確認画面のみ 
//				.anyRequest().authenticated(); // それ以外のパスは認証が不要

		http.authorizeRequests() // 認可に関する設定 TODO:permitAll()を下にする。
				.antMatchers("/item/addindex").authenticated() // ログインが必要なのは、注文確認画面のみ
		...
		http.formLogin() // ログインに関する設定
				...
				.defaultSuccessUrl("/itemlist/page", true)
				// 第1引数:デフォルトでログイン成功時に遷移させるパス //分岐可能？？？
				// 第2引数: true :認証後常に第1引数のパスに遷移
				// false:認証されてなくて一度ログイン画面に飛ばされてもログインしたら指定したURLに遷移
				.usernameParameter("mail") // 認証時に使用するユーザ名のリクエストパラメータ名(今回はメールアドレスを使用)
				.passwordParameter("password"); // 認証時に使用するパスワードのリクエストパラメータ名

		http.logout() // ログアウトに関する設定
				.logoutRequestMatcher(new AntPathRequestMatcher("/logout")) // ログアウトさせる際に遷移させるパス
				.logoutSuccessUrl("/itemlist/page") // ログアウト後に遷移させるパス(ここではログイン画面を設定)
